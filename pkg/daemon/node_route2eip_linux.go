...
func (c *Controller) addNodeIPToLink(link netlink.Link) error {
	...
	if err != nil {
		return fmt.Errorf("failed to parse node IP %s: %w", nodeIP, err)
	}
	if err := netlink.AddrAdd(link, addr); err != nil && !errors.Is(err, syscall.EEXIST) {
		return fmt.Errorf("failed to add address %s to link %s: %w", nodeIP, link.Attrs().Name, err)
	}
	return nil
}
...
func addEIPRoute(eip, cidr string) error {
	...
	link, err := netlink.LinkByName(macvlanName)
	if err != nil {
		return fmt.Errorf("failed to get macvlan sub-interface %s: %w", macvlanName, err)
	}
	...
	if err := netlink.RouteReplace(route); err != nil {
		return fmt.Errorf("failed to add route for EIP %s via %s: %w", eip, macvlanName, err)
	}
...
// hasNatGwPodOnLocalNode checks if the NAT GW pod for the given EIP is running on the local node.
...
func (c *Controller) handleAddIptablesEipRoute(eipName string) error {
	...
	// Only add routes on the node where NAT GW pod is running
	if !c.hasNatGwPodOnLocalNode(eip) {
		klog.V(3).Infof("NAT GW pod for iptables-eip %s not on local node, skipping", eipName)
		return nil
	}
	...
	if err != nil {
		return fmt.Errorf("failed to get subnet %s: %w", eip.Spec.ExternalSubnet, err)
	}

	// Skip if subnet doesn't have macvlan master interface annotation
	if subnet.Annotations[util.NadMacvlanMasterAnnotation] == "" {
		klog.V(3).Infof("subnet %s has no nad-macvlan-master annotation, skipping", subnet.Name)
		return nil
	}
