...
var _ = framework.OrderedDescribe("[group:iptables-vpc-nat-gw]", func() {
	...
	framework.ConformanceIt("[6] node local EIP access via macvlan sub-interface", func() {
		...
		// The macvlan interface name is generated from the external subnet CIDR
		// Get the external subnet CIDR from the shared subnet
		...
		framework.Logf("External subnet CIDR: %s", externalCIDR)
		...
		// e.g., "192.168.1.0/24" -> "macvl19216810"
		cidrParts := strings.Split(externalCIDR, "/")
		networkAddr := strings.ReplaceAll(cidrParts[0], ".", "")
		networkAddr = strings.ReplaceAll(networkAddr, ":", "")
		expectedMacvlanName := "macvl" + networkAddr
		if len(expectedMacvlanName) > 15 {
			expectedMacvlanName = expectedMacvlanName[:15]
		}
		framework.Logf("Expected macvlan interface name: %s", expectedMacvlanName)
